#!/bin/bash
# CASA-XLS-R — acoustic-encoder ablation. Replaces the whisper-medium ACOUSTIC ENCODER with
# facebook/wav2vec2-xls-r-300m (self-supervised, multilingual wav2vec2), while the ASR transcript stays whisper-medium
...
#   bash scripts/train_casa_xlsr.sh
...
python train.py \
    ...
    --whisper_name facebook/wav2vec2-xls-r-300m \
    ...
    --agg_layers 2 \
    --attn_impl sdpa \
    --output_dir checkpoints/casa-xlsr \
