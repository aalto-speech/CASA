#!/bin/bash
# CASA-XLSR-en — acoustic-encoder ablation. Replaces the whisper-medium ACOUSTIC ENCODER with
# jonatasgrosman/wav2vec2-large-xlsr-53-english (wav2vec2 XLSR-53 fine-tuned for English ASR), while the ASR transcript stays whisper-medium
...
#   bash scripts/train_casa_xlsr_en.sh
...
python train.py \
    ...
    --whisper_name jonatasgrosman/wav2vec2-large-xlsr-53-english \
    ...
    --agg_layers 2 \
    --attn_impl sdpa \
    --output_dir checkpoints/casa-xlsr-en \
    --project_name sandi_scorer \
    --run_name casa-xlsr-en \
    --epochs 5 \
    --seed 1011 \
